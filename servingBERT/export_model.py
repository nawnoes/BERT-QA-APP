...
## Other parameters
flags.DEFINE_string("train_file", "config/KorQuAD_v1.0_train.json",
...
#train_batch_size는 최초 32에서 4로 변경
...
class SquadExample(object):
  ...
  """
  #read_squad_examples에서 context데이터와 qas 데이터를 정제하여
  #값을 던달 받는
  def __init__(self,
               qas_id,
               question_text,
...
def read_squad_examples(input_file, is_training):
  ...
  # input_file로 지정한 파일을 json형식으로 로드
  with tf.gfile.Open(input_file, "r") as reader:
    input_data = json.load(reader)["data"]

  #화이트 스페이스를 확인하는 함수 정이
  def is_whitespace(c):
  ...
  for entry in input_data:
    ...
    for paragraph in entry["paragraphs"]:
      ...
      for qa in paragraph["qas"]:
        qas_id = qa["id"] # 질의의 id
        question_text = qa["question"] #질문 데이터
        start_position = None
        end_position = None
        orig_answer_text = None
        is_impossible = False
        if is_training:
          ...
          if not is_impossible:
            ...
            # context 토큰에서 시작과 끝번호에 해당하는 토큰을 가지고 온다
            actual_text = " ".join(
                doc_tokens[start_position:(end_position + 1)])
            cleaned_answer_text = " ".join(
                tokenization.whitespace_tokenize(orig_answer_text)) # orig_answer_text에 대해 처리
            if actual_text.find(cleaned_answer_text) == -1:
              #actual_text내에서 cleaned_answer_text를 찾을수 없는 경
...
def convert_examples_to_features(examples, tokenizer, max_seq_length,
  ...
  for (example_index, example) in enumerate(examples):
    #wordpiece 토크나이저로 토큰 진행
    query_tokens = tokenizer.tokenize(example.question_text)

    #기본 max_query_length 값은 64
    if len(query_tokens) > max_query_length:
      #쿼리 토큰 길이가 최대 쿼리 토큰 길이 보다 길다면 끝 부분을 자른다.
    ...
    #example내 doc_token은 context의 내용을 토큰값으로 가지고 있다.
...
def main(_):
  ...
  if FLAGS.do_train:
    #read_squad_example을 통해서 train 데이터를 변환하여, 질문과 응답을 받는다.
    # context는 제외
  ...
  #export_dir_base 경로에 model export
  estimator.export_saved_model(
      export_dir_base='./savemodel',
